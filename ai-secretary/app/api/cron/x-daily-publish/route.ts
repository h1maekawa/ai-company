...
import { DAILY_X_LOCK_TTL_SEC, runDailyXAutomation } from "@/app/lib/note/automation/dailyX";
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    const result = await withLock("daily-x-publish", runDailyXAutomation, { ttlSec: DAILY_X_LOCK_TTL_SEC });
