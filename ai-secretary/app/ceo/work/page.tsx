  ...
  return (
    <main className="mx-auto w-full max-w-3xl overflow-x-hidden px-4 py-5">
      ...
      <div className="space-y-3">
        {BUSINESS_DEPARTMENT_NAV.map((item) => <Link key={item.id} href={item.href} className="block min-h-16 rounded-2xl border border-slate-800 bg-slate-900/70 p-4"><strong>{item.icon} {item.label}</strong><span className="mt-1 block text-sm text-slate-400">{item.description}</span></Link>)}
      </div>
      <h2 className="mb-3 mt-7 text-lg font-bold">全社共有基盤</h2>
