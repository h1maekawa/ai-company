...
export default function OrganizationPage() {
  ...
  const load = useCallback(async () => {
    try {
      const [proposalResponse, candidateResponse, effectivenessResponse] = await Promise.all([fetch("/api/company/proposals"), fetch("/api/company/skill-candidates"), fetch("/api/company/skill-effectiveness")]);
      if (!proposalResponse.ok || !candidateResponse.ok || !effectivenessResponse.ok) throw new Error();
      const [proposalJson, candidateJson, effectivenessJson] = await Promise.all([proposalResponse.json(), candidateResponse.json(), effectivenessResponse.json()]);
