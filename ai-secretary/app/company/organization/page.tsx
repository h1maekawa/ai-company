...
type EffectivenessView = { definition: { id: string; name: string; status: string }; metrics: { availability: string; executionCount: number | null; successCount: number | null; failureCount: number | null; successRate: number | null; distinctMissionCount: number | null; agents: string[]; lastUsedAt: string | null; averageDurationMs: number | null; failureBreakdown: Record<string, number>; frequentlyUsedKnowledge: Array<{ id: string; count: number }> } };
type ImprovementView = { id: string; skillId: string; reasonType: string; status: string; evidence: { executionCount: number; successCount: number; failureCount: number; distinctMissionCount: number; errorCodes: string[] } };
type ResearchHealthView = { departmentId:string;status:string;lastSuccessfulRun:string|null;lastPartialRun:string|null;lastFailure:string|null;freshItemCount:number;staleItemCount:number;providerFailures:string[] };
type CompanyImprovementView = {id:string;category:string;title:string;problem:string;evidence:string[];affectedDepartmentIds:string[];severity:string;risk:string;status:string};
...
export default function OrganizationPage() {
  ...
  const [effectiveness, setEffectiveness] = useState<EffectivenessView[]>([]);
  const [improvements, setImprovements] = useState<ImprovementView[]>([]);
  const [researchHealth, setResearchHealth] = useState<ResearchHealthView[]|null>(null);
  const [companyImprovements, setCompanyImprovements] = useState<CompanyImprovementView[]|null>(null);
  const [error, setError] = useState("");

  const load = useCallback(async () => {
    try {
      const [proposalResponse, candidateResponse, effectivenessResponse,researchResponse,operationalResponse] = await Promise.all([fetch("/api/company/proposals"), fetch("/api/company/skill-candidates"), fetch("/api/company/skill-effectiveness"),fetch("/api/company/research"),fetch("/api/company/operational-health")]);
      if (!proposalResponse.ok || !candidateResponse.ok || !effectivenessResponse.ok) throw new Error();
      const [proposalJson, candidateJson, effectivenessJson,researchJson,operationalJson] = await Promise.all([proposalResponse.json(), candidateResponse.json(), effectivenessResponse.json(),researchResponse.ok?researchResponse.json():null,operationalResponse.ok?operationalResponse.json():null]);
      ...
      setEffectiveness(effectivenessJson.effectiveness ?? []); setImprovements(effectivenessJson.improvementCandidates ?? []);
      setResearchHealth(researchJson?.health??null);setCompanyImprovements(operationalJson?.improvementCandidates??null);
  ...
  return (
    <main className="mx-auto max-w-4xl space-y-4 px-3 py-6 sm:px-6">
      ...
      <section className="space-y-3"><div><h2 className="text-sm font-semibold text-white">Research Health</h2><p className="text-[11px] text-sub">Source付きResearchの鮮度とProvider障害。Researchは正式Knowledgeではありません。</p></div>{researchHealth===null?<p className="rounded-xl border border-hairline p-3 text-xs text-sub">Research Health: UNKNOWN</p>:<div className="grid gap-3 sm:grid-cols-2">{researchHealth.map((health)=><article key={health.departmentId} className="rounded-2xl border border-hairline bg-ink-card p-4"><div className="flex justify-between"><strong>{health.departmentId}</strong><span className="text-xs text-brand">{health.status}</span></div><p className="mt-2 text-xs text-sub">Fresh {health.freshItemCount} · Stale {health.staleItemCount}</p><p className="mt-1 text-[10px] text-sub">Last success: {health.lastSuccessfulRun??"UNKNOWN"}</p>{health.providerFailures.length?<p className="mt-1 text-[10px] text-loss">Failed: {health.providerFailures.join(", ")}</p>:null}</article>)}</div>}</section>

      <section id="company-improvements" className="space-y-3"><div><h2 className="text-sm font-semibold text-white">Company Improvement Candidates</h2><p className="text-[11px] text-sub">Repeated operational evidenceのみ。承認してもコード変更やEngineering開始は行いません。</p></div>{companyImprovements===null?<p className="rounded-xl border border-hairline p-3 text-xs text-sub">Operational evidence: UNKNOWN</p>:companyImprovements.length===0?<p className="rounded-xl border border-hairline p-3 text-xs text-sub">十分な反復Evidenceを持つ候補はありません。</p>:companyImprovements.map((item)=><article key={item.id} className="rounded-2xl border border-hairline bg-ink-card p-4"><div className="flex justify-between"><strong>{item.title}</strong><span className="text-xs text-warning">{item.severity} · {item.risk}</span></div><p className="mt-2 text-xs text-sub">{item.problem}</p><p className="mt-2 text-[10px] text-sub">Evidence {item.evidence.length} · {item.affectedDepartmentIds.join(", ")||"company-wide"}</p>{item.status==="PROPOSED"?<div className="mt-3 flex gap-2">{(["REJECTED","HOLD","APPROVED"] as const).map((decision)=><button key={decision} onClick={()=>void humanAction(`/api/company/improvements/${encodeURIComponent(item.id)}/decision`,`改善候補を${decision}として記録します。自動変更は行いません。`,{decision})} className="min-h-11 rounded-lg border border-hairline px-3 text-xs">{decision}</button>)}</div>:<p className="mt-2 text-xs">{item.status}</p>}</article>)}</section>
      ...
      <section className="space-y-3"><div><h2 className="text-sm font-semibold text-white">Skill Effectiveness</h2><p className="text-[11px] text-sub">Execution Factのみを表示し、Mission成功との因果は断定しません。</p></div><div className="grid gap-3 sm:grid-cols-2">{effectiveness.filter((item)=>item.definition.status==="implemented").map((item)=><article key={item.definition.id} className="rounded-2xl border border-hairline bg-ink-card p-4"><div className="flex justify-between"><strong className="text-sm">{item.definition.name}</strong><span className="text-[10px] text-brand">IMPLEMENTED</span></div>{item.metrics.availability==="UNKNOWN"?<p className="mt-3 text-xs text-sub">Usage: UNKNOWN</p>:<div className="mt-3 grid grid-cols-2 gap-2 text-xs"><p>Executions<br/><strong>{item.metrics.executionCount}</strong></p><p>Distinct Missions<br/><strong>{item.metrics.distinctMissionCount}</strong></p><p>Success<br/><strong>{item.metrics.successRate==null?"UNKNOWN":`${Math.round(item.metrics.successRate*100)}%`}</strong></p><p>Avg Duration<br/><strong>{item.metrics.averageDurationMs==null?"UNKNOWN":`${Math.round(item.metrics.averageDurationMs)}ms`}</strong></p><p className="col-span-2">Agents: {item.metrics.agents.join(", ")||"なし"}</p><p className="col-span-2">Last Used: {item.metrics.lastUsedAt??"確認済み0件"}</p><p className="col-span-2">Knowledge: {item.metrics.frequentlyUsedKnowledge.map((knowledge)=>knowledge.id).join(", ")||"なし"}</p></div>}</article>)}</div>{improvements.length?<div className="space-y-2"><h3 className="text-xs font-semibold text-warning">Skill改善候補</h3>{improvements.map((item)=><div key={item.id} className="rounded-xl border border-warning/30 p-3 text-xs"><strong>{item.skillId}</strong><p>{item.reasonType}: {item.evidence.failureCount}/{item.evidence.executionCount} failures</p><p>Human Review required. 自動修正・削除・Engineering開始は行いません。</p>{item.status==="PROPOSED"?<div className="mt-2 flex gap-2">{(["REJECTED","HOLD","APPROVED"] as const).map((decision)=><button key={decision} onClick={()=>void humanAction(`/api/company/skill-improvements/${encodeURIComponent(item.id)}/decision`,`改善候補を${decision}として記録します。Code変更は行いません。`,{decision})} className="min-h-11 rounded-lg border border-hairline px-3">{decision}</button>)}</div>:null}</div>)}</div>:null}</section>
