...
export type ResearchQuery = { departmentId: string; researcherAgentId: string; topic: string; maxItems: number; since?: string };
...
export type ResearchProviderResult = { items: ResearchProviderItem[]; warnings?: string[] };
...
export type ResearchRun = { id: string; departmentId: string; researcherAgentId: string; startedAt: string; completedAt?: string; queryCount: number; fetchedCount: number; acceptedCount: number; duplicateCount: number; staleCount: number; failedSources: string[]; status: "RUNNING" | "COMPLETED" | "PARTIAL" | "FAILED" };
