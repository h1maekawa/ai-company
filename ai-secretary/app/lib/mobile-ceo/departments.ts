...
import { deriveCreatorWorkflowProgress } from "@/app/lib/company/execution/creatorWorkflow";
import { creatorRevenueAttention, nextReviewAt, thesisAlertCount, type AttentionCard } from "@/app/lib/mobile-ceo/controlCenter";
...
export type DepartmentReadModel = {
  ...
  /** CEO確認カード。problems は ACTION_REQUIRED だけを文字列化した互換field。 */
  attention?: AttentionCard[];
  /** 実在Missionではない事業機会。トップの主役にせず「その他」に置く。 */
  opportunities?: string[];
  executionAuthority?: "HUMAN_ONLY"; aiExecutionAllowed?: false;
  workflows?: Array<{ id: string; title: string; status: string; completed: number; total: number; steps: Array<{ id: string; title: string; status: string; assignedAgentId?: string; dependsOn: string[]; inputRefs: string[]; outputRefs: string[]; knowledgeRefs: string[] }> }>;
};
...
export function buildDepartmentReadModel(id: DepartmentId, data: Json, generatedAt = new Date().toISOString()): DepartmentReadModel {
  ...
  if (id === "creator") {
    ...
    const missions = arr(data.execution?.state?.missions); const plans = arr(data.execution?.state?.plans); const workflows = missions.flatMap((mission) => { const plan = plans.find((candidate) => candidate.id === mission.executionPlanId && candidate.workflowKind === "CREATOR_MULTI_AGENT"); if (!plan) return []; const progress = deriveCreatorWorkflowProgress(plan); return [{ id: mission.id, title: mission.title, status: progress.status, completed: progress.completed, total: progress.total, steps: plan.steps.map((step: Json) => ({ id: step.id, title: step.title, status: step.status, assignedAgentId: step.assignedAgentId, dependsOn: step.dependsOn ?? [], inputRefs: step.inputRefs ?? [], outputRefs: step.outputRefs ?? [], knowledgeRefs: step.knowledgeRefs ?? [] })) }]; });
    // Revenue未記録（UNKNOWN）はKPI欄の「未取得」であり、単独ではCEO確認にしない。
    const attention = creatorRevenueAttention({ economicsAvailable: data.economics !== null && data.economics !== undefined, workflows });
    return { id, name: "Creator", northStar: metric("creator_profit", "Creator Profit", outcome.profitYen, "Company Revenue + Cost Ledger", "円", status), outcomes: [metric("revenue", "Revenue", revenue, "Revenue Ledger", "円", outcome.revenueStatus), metric("profit", "Profit", outcome.profitYen, "Economic Outcome", "円", status), metric("roi", "ROI", outcome.roi, "Economic Outcome", "x", status), contentStamp(metric("conversions", "Conversions", content.conversions, "Content Ledger")), metric("revenue_per_content", "Revenue / Content", revenuePerContent, "Revenue + Content", "円")], operations: [["published", "Published", content.publishedCount], ["x_impressions", "X Impressions", content.xImpressions], ["note_views", "Note Views", content.noteViews], ["link_clicks", "Link Clicks", content.linkClicks], ["cta_clicks", "CTA Clicks", content.ctaClicks], ["ctr", "CTR", content.derived?.ctr], ["cta_ctr", "CTA CTR", content.derived?.ctaCtr], ["conversion_rate", "Conversion Rate", content.derived?.conversionRate], ["rpm", "Revenue / 1000 Impressions", content.derived?.revenuePer1000Impressions]].map(([k,l,v]) => contentStamp(metric(String(k), String(l), v, "Content Dashboard"))), workflows, currentWork: workflows.filter((workflow) => workflow.status !== "COMPLETED").slice(0, 5).map((workflow) => workflow.title), opportunities: opportunities.slice(0, 5).map((x) => String(x.title ?? x.name ?? "Creator opportunity")), attention, problems: attention.filter((card) => card.type === "ACTION_REQUIRED").map((card) => card.title), suggestions: [] };
  }
  if (id === "fund") {
    const fund = data as FundPayload & Json; const perf = data.performance?.performance ?? data.transactions?.performance ?? {}; const recs = fund.recommendations?.recommendations ?? []; const decisions = arr(data.decisions?.decisions); const tx = arr(data.transactions?.transactions); const learnings = arr(data.learning?.learnings); const thesisAlerts = thesisAlertCount(Array.isArray(data.outcomes?.outcomes) ? data.outcomes.outcomes : null); const review = nextReviewAt(recs, new Date(generatedAt)); const portfolio = fund.portfolio; const portfolioAsOf = portfolio?.freshness?.asOf ?? portfolio?.revaluedAt ?? portfolio?.updatedAt; const portfolioStamp = stamp(portfolioAsOf); const recommendationTimes = recs.map((r) => r.dataAsOf).filter((v) => v && v !== "unknown").sort(); const recommendationAsOf = recommendationTimes[recommendationTimes.length - 1];
