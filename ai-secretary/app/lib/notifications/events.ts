...
export function buildNotificationEvents(state: ExecutionState, engineering?: EngineeringNotifications): NotificationEvent[] {
  const approvals = state.approvals.filter((item) => item.status === "PENDING").map((approval): NotificationEvent => {
    ...
    const contentDraftRegistration = target?.startsWith("content-candidate:") ?? false;
    return { id: id(fingerprint), fingerprint, kind: contentDraftRegistration ? "CONTENT_DRAFT_READY" : "COMPANY_APPROVAL_REQUIRED", sourceType: "approval", sourceId: approval.id, title: approval.title, summary: approval.summary, priority: approval.riskLevel === "R4" ? "URGENT" : "ACTION_REQUIRED", actionRequired: true, deepLink: `/ceo/approvals#approval-${encodeURIComponent(approval.id)}`, createdAt: approval.createdAt, riskLevel: approval.riskLevel, approvalKind: contentDraftRegistration ? "CONTENT_DRAFT_REGISTRATION" : "ACTION" };
  });
