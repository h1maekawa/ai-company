...
function MetricCard({ item }: { item: DepartmentMetric }) { return <div className="min-w-0 rounded-xl border border-slate-800 bg-slate-950/60 p-3"><div className="text-xs text-slate-400">{item.label}</div><div className="mt-1 break-words text-lg font-bold">{item.value === null ? "UNKNOWN" : `${item.value.toLocaleString("ja-JP")}${item.unit ?? ""}`}</div><div className="mt-1 text-[10px] text-slate-500">{item.availability} · {item.source}</div></div>; }
